...
OUTPUT_FILE = "dados_tendencia_bdr.json"


def scanner_tendencia_bdr():
    try:
        ...
        for subcats in estrutura.get('bdrs', {}).values():
            for ativo in subcats:
                ticker = ativo.get('ticker', '').strip().upper()
                if not ticker:
                    continue
                tickers_bdrs.append(ticker)
                mapa_nomes[ticker] = ativo.get('nome', ticker)

        if not tickers_bdrs:
            print("Nenhum BDR encontrado para scanner.")
            return

        print(f"Iniciando Scanner BDR para {len(tickers_bdrs)} ativos...")
        df = yf.download(tickers_bdrs, period="1y", progress=False, auto_adjust=True)
        precos = df['Close']
        volumes = df['Volume']
        ...
        print('Sucesso: dados_tendencia_bdr.json gerado com novos filtros de qualidade.')
